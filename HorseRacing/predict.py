...
import os
import re
# matplotlib + seaborn are only needed for the per-race PNG/HTML report
# (skipped when PREDICT_SKIP_VISUALS=1). Make the imports optional so the
# production workflow doesn't have to install them.
try:
...
def main():
    ...
    # Stash identifier columns before they're dropped from the model input,
    # so the predictions output can carry them through (used for backtest joins).
    _id_stash = {}
    for _id_col in ('track_code', 'race_date'):
        if _id_col in df.columns:
            _id_stash[_id_col] = df[_id_col].copy()

    print(f"Dropping unnecessary columns: {', '.join([c for c in columns_to_drop if c in df.columns])}")
    df = df.drop(columns=[col for col in columns_to_drop if col in df.columns], errors='ignore')

    for _id_col, _series in _id_stash.items():
        df[_id_col + '_stash'] = _series
    ...
    X = df.drop(columns=['race_id', 'original_horse_name',
                         'track_code_stash', 'race_date_stash'], errors='ignore')
    ...
    # Pass-through identifiers (stashed before the model-input column drop)
    # needed for backtest joins. Harmless when absent in production use.
    if 'track_code_stash' in df.columns:
        predictions_df['track_code'] = df['track_code_stash']
    if 'race_date_stash' in df.columns:
        predictions_df['race_date'] = df['race_date_stash']
    ...
    # Production deploys (the GitHub Action that powers the UI) only consume
    # all_race_predictions.csv. The per-race CSVs, PNGs, and HTML report below
    # are useful for local debugging but add ~5-10s and force matplotlib/seaborn
    # into the install. Set PREDICT_SKIP_VISUALS=1 in the workflow to skip.
    if os.environ.get('PREDICT_SKIP_VISUALS', '').strip() in ('1', 'true', 'True'):
        print("PREDICT_SKIP_VISUALS=1 — skipping per-race CSVs/PNGs and HTML report")
